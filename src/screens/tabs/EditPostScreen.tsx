...
const EditPostScreen = ({ route }) => {
  ...
  const createFormData = () => {
    ...
    if (contentType === 'reel') {
      ...
      const trimSuffix = trimmedVideo ? '_trimmed' : '';
      const fileName = generateFileName(videoUri, detectedMimeType, 'reel', trimSuffix);
          ...
          {videoAsset?.fileSize && (
            <View style={styles.fileSizeIndicator}>
              <Text style={styles.fileSizeText}>
                {Math.round(videoAsset.fileSize / (1024 * 1024))}MB
              </Text>
            </View>
          )}
        </View>

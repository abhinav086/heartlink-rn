// src/screens/profile/FollowersList.js
...
const FollowersList = ({ route, navigation }) => {
  const { userId, username: profileUsername } = route.params; // The user whose followers we are listing
  ...
  const fetchFollowers = async (isRefresh = false) => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/v1/users/followers/${userId}?page=${currentPage}&limit=20`, {
        method: 'GET',
        headers: {
      ...
      setError(e.message || "Failed to load followers. Please try again.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{profileUsername}'s Followers</Text>
      ...
      ) : followers.length === 0 && !loading && !refreshing ? (
        <View style={styles.messageContainer}>
